...
class contractDynamicFields(models.TransientModel):
    ...
    def set_domain(self):
        ...
        model_id = self.env['ir.model'].sudo().search([('model', '=', 'hr.contract')])
    ...
    ref_model_id = fields.Many2one('ir.model', string="Model d'objet", index=True)
    rel_field = fields.Many2one('ir.model.fields', string='Relation')
    ttype = fields.Selection(selection='get_field_types', string='Type de champ', required=True)
